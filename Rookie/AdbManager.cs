        ...
        public static ProcessOutput Sideload(string path, string packagename = "")
        {
            ...
            if (out2.Contains("failed"))
            {
                ...
                if (out2.Contains($"signatures do not match previously") || out2.Contains("INSTALL_FAILED_VERSION_DOWNGRADE") || out2.Contains("signatures do not match") || out2.Contains("failed to install"))
                {
                    ...
                    _ = AdbManager.RunAdbCommandToString("kill-server");
                    _ = AdbManager.RunAdbCommandToString("devices");
                    _ = AdbManager.RunAdbCommandToString($"pull \"/sdcard/Android/data/{MainForm.CurrPCKG}\" \"{Environment.CurrentDirectory}\"");
                    ...
                    ret += AdbManager.RunAdbCommandToString($"install -g \"{path}\"");
                    _ = AdbManager.RunAdbCommandToString($"push \"{Environment.CurrentDirectory}\\{MainForm.CurrPCKG}\" /sdcard/Android/data/");
